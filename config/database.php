...
// Ortamı dinamik olarak algıla (Yerelde çalışıyorsa 'local', sunucudaysa 'production')
define('APP_ENV', detectEnvironment());

// Veritabanı ayarları
if (APP_ENV === 'local') {
    // LOCAL (XAMPP/WAMP/Laragon) AYARLARI
    define('DB_HOST', 'localhost');
    define('DB_PORT', '3306');
    define('DB_USER', 'root');          // Yerel MySQL varsayılan kullanıcı adı (genelde root)
    define('DB_PASS', '');              // Yerel MySQL varsayılan şifresi (boş bırakılır)
    define('DB_NAME', 'navexmar');      // Yerel veritabanı adı (database_complete.sql'deki ad)
} else {
